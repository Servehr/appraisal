...
export const MultiOption = ({color='primary', labelText, title, size='md', label=false, type, htmlFor=''}: Properties)  =>
{
    return (
        <>
            { label ?? <label htmlFor={htmlFor} className="col-12 col-form-label">{labelText}</label>}
            <select className="form-select" multiple aria-label="multiple select example" id='' name=''>
